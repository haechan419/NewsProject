...
/**
 * 게시판 더미 데이터 초기화 클래스
 * User1부터 User300까지의 회원과 각 회원당 여러 개의 게시글을 생성합니다.
 */
@Component
@Order(2) // Member 초기화(Order=1) 이후에 실행
@Slf4j
...
public class BoardDataInitializer implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) throws Exception {
        ...
        for (int i = 1; i <= 300; i++) {
            ...
            // 먼저 nickname으로 찾기 (이미 존재하는 회원)
            Member member = memberRepository.findByNickname(nickname)
                .orElse(null);
            
            // nickname으로 못 찾으면 email로 찾기
            if (member == null) {
            ...
            // 둘 다 없으면 생성
            if (member == null) {
                log.warn("회원을 찾을 수 없어 생성합니다: nickname={}, email={}", nickname, email);
        ...
        log.info("회원 준비 완료: {}명 (기존: {}명, 신규: {}명)", members.size(), foundCount, notFoundCount);
        
        if (members.isEmpty()) {
            log.error("회원이 없어 게시글을 생성할 수 없습니다!");
            return;
        }
        ...
        for (Member member : members) {
            // 각 회원당 랜덤하게 3~10개의 게시글 생성
            int boardCount = random.nextInt(8) + 3; // 3~10개
            
            if (member == null || member.getId() == null) {
                log.warn("유효하지 않은 회원 건너뜀: {}", member);
                continue;
            }
            ...
            for (int j = 0; j < boardCount; j++) {
                ...
                // 제목과 내용 랜덤 선택
                String title = titles[random.nextInt(titles.length)] + " - " + member.getNickname();
                ...
                try {
                    ...
                    if (savedBoard != null && savedBoard.getId() != null) {
                        totalBoards++;
                        ...
                        // 100개마다 진행 상황 로그
                        if (totalBoards % 100 == 0) {
                            log.info("게시글 생성 진행 중... {}개 생성됨", totalBoards);
                        }
                    } else {
                        log.error("게시글 저장 실패: member={}, title={}", member.getNickname(), title);
                    }
                } catch (Exception e) {
                    log.error("게시글 저장 중 오류 발생: member={}, title={}, error={}", 
                        member.getNickname(), title, e.getMessage(), e);
                }
            }
            
            processedMembers++;
            if (processedMembers % 50 == 0) {
                log.info("회원 처리 진행 중... {}/{}명 완료", processedMembers, members.size());
            }
        }

        log.info("=== 게시판 더미 데이터 초기화 완료 ===");
        log.info("생성된 게시글 수: {}개", totalBoards);
        log.info("일반 게시판: {}개", normalBoards);
        log.info("토론 게시판: {}개", debateBoards);
        log.info("회원 수: {}명", members.size());
    }
}
