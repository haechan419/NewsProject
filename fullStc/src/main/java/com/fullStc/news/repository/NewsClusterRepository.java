...
public interface NewsClusterRepository extends JpaRepository<NewsCluster, Long> {
    ...
    // 카테고리 목록으로 클러스터 조회 (cluster_summary가 있는 것만)
    @Query(value = "SELECT * FROM news_cluster WHERE category IN :categories AND cluster_summary IS NOT NULL AND cluster_summary != '' ORDER BY updated_at DESC LIMIT :limit", nativeQuery = true)
    List<NewsCluster> findByCategoriesOrderByUpdatedAtDesc(@Param("categories") List<String> categories, @Param("limit") int limit);

    // 최신 클러스터 조회 (cluster_summary가 있는 것만)
    @Query(value = "SELECT * FROM news_cluster WHERE cluster_summary IS NOT NULL AND cluster_summary != '' ORDER BY updated_at DESC LIMIT :limit", nativeQuery = true)
    List<NewsCluster> findLatestClusters(@Param("limit") int limit);

    // 카테고리 목록으로 클러스터 조회 (cluster_summary 조건 없이)
    @Query(value = "SELECT * FROM news_cluster WHERE category IN :categories ORDER BY updated_at DESC LIMIT :limit", nativeQuery = true)
    List<NewsCluster> findByCategoriesWithoutSummaryCheck(@Param("categories") List<String> categories, @Param("limit") int limit);
}