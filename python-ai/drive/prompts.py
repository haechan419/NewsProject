...
사용자의 음성 명령을 듣고 다음 중 하나의 의도로 분류하세요.

가능한 의도:
# 플레이리스트 선택 화면에서
- SELECT_INTEREST: 오늘의 관심 뉴스 선택 ("관심 뉴스", "관심 카테고리", "내 관심 뉴스")
- SELECT_LATEST: 오늘의 주요 뉴스 선택 ("주요 뉴스", "최신 뉴스", "오늘 뉴스")
- SELECT_ECONOMY: 경제·비즈니스 뉴스 선택 ("경제 뉴스", "비즈니스 뉴스", "경제")
- SELECT_POLITICS: 정치·사회 뉴스 선택 ("정치 뉴스", "사회 뉴스", "정치")
- SELECT_IT: IT·과학 뉴스 선택 ("IT 뉴스", "과학 뉴스", "IT")
- SELECT_HOT: 긴급 속보 선택 ("긴급 속보", "속보", "긴급 뉴스")

# 플레이리스트 재생 중
- PAUSE: 재생 일시정지 ("일시정지", "멈춰", "정지")
- RESUME: 재생 재개 ("재생", "계속", "다시")
- SEEK_FORWARD: 10초 앞으로 이동 ("앞으로", "10초 앞으로", "빨리 감기")
- SEEK_BACKWARD: 10초 뒤로 이동 ("뒤로", "10초 뒤로", "되돌리기")
- STOP: 드라이브 모드 종료 ("종료", "끝", "나가")
- HELP: 도움말 ("도움말", "명령어", "어떻게", "사용법")
- HISTORY_OPEN: 히스토리 열기 ("히스토리", "기록", "들었던 뉴스")

사용자 명령: {command_text}

중요:
1. 명령어가 명확하지 않으면 confidence를 낮게 설정하세요 (0.5 이하)
2. 여러 의도가 섞여 있으면 가장 중요한 의도를 선택하세요
3. "다음", "넘어가", "스킵" 같은 짧은 명령은 NEXT로 분류하세요
